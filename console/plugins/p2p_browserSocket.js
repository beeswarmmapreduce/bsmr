...
function p2pPlugin(params, onData, onError)
	...
	this.fetchIntermediateData = function(jobId, partitionId, splitId, location)
		...
		worker.log('p2p:open: ' + partitionId + ', ' + splitId + ', '
				+ location, 'log', LOG_INFO);
		...
		ws.onmessage = function(e)
			...
			if (msg)
				...
				if (msg.payload.action == 'upload')
					{
					onData(msg.payload.jobId, msg.payload.partitionId,
							msg.payload.splitId, msg.payload.data);
					}
				}
			ws.close();
			}
		ws.onclose = function()
			{
			worker.log('p2p:close: ' + partitionId + ', ' + splitId + ', '
					+ location, 'log', LOG_INFO);
			if (!msg)
				{
		...
		ws.onerror = function()
			{
			worker.log('p2p:error: ' + partitionId + ', ' + splitId + ', '
					+ location, 'log', LOG_INFO);
			onError(jobId, partitionId, splitId, location);
			}
		}
